# --------------------------------------------------------
# Code for a Ordered Logit with Random Coefficients and
# Fixed Thresholds
# 
...
choicedata <- read.table("ol_simulated.csv", sep = ",", header = TRUE)
...
modelname <- "Ordered_Logit"		# used for output
...
likelihood <- function(fc, b) {
     ...
     p <- (    1 / (1 + exp(v1 - threshold1))                                  ) * choice1 +
          (    1 / (1 + exp(v1 - threshold2)) -(1 / (1 + exp(v1 - threshold1)))) * choice2 + 
          (1 - 1 / (1 + exp(v1 - threshold2))                                  ) * choice3
